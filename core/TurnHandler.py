...
class TurnHandler:
    ...
    def Check_Enemy_or_familiar_Unit(self,world:World,unit:Unit,enemy_familiar:bool)->bool:
        ...
        if(enemy_familiar==False):
            if(All_units_enemy.__contains__(unit)):return True
            else:return False
        else:
            if(All_units_own.__contains__(unit)):return True
            else:return False
    ...
    def Manhatan_Distance(self,BeginingCell:Cell,EndingCell:Cell)->int:
        import math
        dist=math.fabs(BeginingCell.row-EndingCell.row)+math.fabs(BeginingCell.col-EndingCell.col)
        return dist
